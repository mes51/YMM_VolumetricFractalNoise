...
cbuffer constants : register(b0)
{
    ...
    float3 noisePosition : packoffset(c2);

    float4x4 noiseRotateAndScale : packoffset(c3);

    float contrast : packoffset(c7.x);

    float density : packoffset(c7.y);

    int octave : packoffset(c7.z);

    float evolution : packoffset(c7.w);

    float3 falloffRegion : packoffset(c8);

    float falloffPower : packoffset(c8.w);

    float3 geometrySize : packoffset(c9);

    float3 geometryPosition : packoffset(c10);

    float4x4 geometryRotation : packoffset(c11);

    float fieldOfView : packoffset(c15.x);

    float castResolution : packoffset(c15.y);

    float distanceAttenuation : packoffset(c15.z);
};
...
float fbm(float3 pos, float evolution, int octave, float4x4 noiseTransform)
{
    pos = mul(noiseTransform, float4(pos, 1.0)).xyz;
    float f = 0.0;
    for (int i = 0; i < octave; i++)
    {
        f += (cnoise(float4(pos, evolution)) * min(1.0, octave - i)) / pow(2.0, i + 1);
        pos *= (2.0 + i * 0.01 + 0.01);
    }
    f /= 1.0 - 1.0 / pow(2.0, octave + 1);

    return abs(f);
}

float3 intersectBox(float3 eye, float3 dir, float3 boxSize)
{
    float3 minPoint = boxSize * -0.5;
    float3 maxPoint = boxSize * 0.5;
    float3 tmin = (minPoint - eye) / dir;
    float3 tmax = (maxPoint - eye) / dir;

    float3 tNear = min(tmin, tmax);
    float3 tFar = max(tmin, tmax);
    
    float maxNearDist = max(tNear.x, max(tNear.y, tNear.z));
    float minFarDist = min(tFar.x, min(tFar.y, tFar.z));

    if (maxNearDist > 0.0 && maxNearDist < minFarDist)
    {
        return float3(1.0, maxNearDist, minFarDist);
    }
    else if (length(min(eye, minPoint) - minPoint) == 0.0 && length(max(eye, maxPoint) - maxPoint) == 0.0)
    {
        return float3(1.0, 0.0, minFarDist);
    }
    else
    {
...
float distanceFromCore(float3 pos, float3 coreSize)
{
    float3 minPoint = coreSize * -0.5;
    float3 maxPoint = coreSize * 0.5;
    ...
    return length((min(pos, minPoint) - minPoint) + (max(pos, maxPoint) - maxPoint));
}

float3 rayDirection(float2 position, float fov)
{
    return normalize(float3(position * 0.5, -1.0 / fov));
}

float renderNoise(float3 eye, float3 dir, float near, float far, float3 pixelScale)
{
    float distance = far - near;
    float iterationDiff = castResolution * pixelScale.x;
    float densityRateByCast = castResolution * 0.1;
    float alpha = 0.0;
    float3 coreSize = max(float3(0.0, 0.0, 0.0), geometrySize - falloffRegion) * pixelScale.xyz;
    
    float4x4 noiseTransform = mul(
        noiseRotateAndScale,
        float4x4(
            float4(1.0, 0.0, 0.0, noisePosition.x * -pixelScale.x),
            float4(0.0, 1.0, 0.0, noisePosition.y * -pixelScale.y),
            float4(0.0, 0.0, 1.0, noisePosition.z * pixelScale.x),
            float4(0.0, 0.0, 0.0, 1.0)
        )
    );

    for (float f = 0.0; f < distance; f += iterationDiff)
    {
        float3 pos = eye + dir * (f + near);
        float nz = fbm(pos, evolution, octave, noiseTransform) * density;

        alpha += nz * min((distance - f) / iterationDiff, 1.0) * densityRateByCast * // ray cast adjustment
            pow(1.0 + distanceFromCore(pos, coreSize) * falloffPower, -1.0) * // fall off
            pow(1.0 + (f + near) * distanceAttenuation, -1.0); // distance attenuation
    }

    return saturate((alpha - 0.5) * contrast + 0.5);
}

float4 main(float4 pos : SV_POSITION, float4 posScene : SCENE_POSITION, float4 uv0 : TEXCOORD0) : SV_Target
{
    float3 pixelScale = uv0.zwz * 2.0;
    float4x4 geometryTransform = mul(
        geometryRotation,
        float4x4(
            float4(1.0, 0.0, 0.0, geometryPosition.x * -pixelScale.x),
            float4(0.0, 1.0, 0.0, geometryPosition.y * -pixelScale.y),
            float4(0.0, 0.0, 1.0, geometryPosition.z * pixelScale.z),
            float4(0.0, 0.0, 0.0, 1.0)
        )
    );
    
    float3 eye = mul(geometryTransform, float4(0.0, 0.0, 0.0, 1.0)).xyz;
    float3 dir = mul(geometryTransform, float4(rayDirection(posScene.xy * pixelScale.xy, fieldOfView), 0.0)).xyz;
    float2 dist = float2(0.0, 0.0);

    float hitRate = 0.0;
    int hitCount = 0;
    float3 boxSize = geometrySize * pixelScale;

    // pseudo super sampling
    float3 result = intersectBox(eye + float3(-0.45, -0.05, 0.0) * pixelScale, dir, boxSize);
    if (result.x > 0.0)
    {
        hitCount++;
        hitRate += 0.25;
    ...
    result = intersectBox(eye + float3(0.05, -0.45, 0.0) * pixelScale, dir, boxSize);
    if (result.x > 0.0)
    {
        hitCount++;
        hitRate += 0.25;
    ...
    result = intersectBox(eye + float3(0.45, 0.05, 0.0) * pixelScale, dir, boxSize);
    if (result.x > 0.0)
    {
        hitCount++;
        hitRate += 0.25;
    ...
    {
        dist /= float(hitCount);
        return lerp(color2, color1, renderNoise(eye, dir, dist.x, dist.y, pixelScale)) * hitRate;
    }
    else
    {
